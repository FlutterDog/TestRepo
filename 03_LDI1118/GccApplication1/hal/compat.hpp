/**
 * @file compat.hpp
 * @brief Общий HAL и базовые системные объявления для восьмибитных AVR.
 *
 * @details
 * Заголовок предоставляет:
 * - базовые проектные типы byte, word, boolean;
 * - константы формата кадра UART SERIAL_*;
 * - служебные функции ограничения и линейного отображения значений;
 * - пространство имён hal для низкоуровневых аппаратных вызовов;
 * - минимальный потоковый интерфейс SerialPort поверх UART0;
 * - глобальные inline-обёртки для часто используемых системных операций.
 *
 * @note
 ...
 * Назначение реле, светодиодов, адресных входов и линии RS-485 описывается
 * в аппаратном слое платы.
 */
...
/*----------------------------------------------------------------------------
 * Защита проектных алиасов от внешних макроопределений
 *---------------------------------------------------------------------------*/
 ...
 * @brief Константы формата кадра UART.
 *
 * @details
 * Значения кодируют количество бит данных, тип паритета и количество
 * стоп-битов. Константы используются при инициализации UART0.
 *
 * @{
 */
#ifndef SERIAL_5N1
...
/**
 * @namespace hal
 * @brief Низкоуровневый аппаратный слой.
 *
 * @details
...
namespace hal
{
     ...
     *
     * @note
     * Проектная нумерация пинов задаётся реализацией GPIO-драйвера.
     * Для сигналов конкретной платы следует использовать аппаратный слой платы.
     */
     ...
     * @param pin Номер пина в проектной нумерации.
     *
     * @return true — логическая единица.
     * @return false — логический ноль.
     */
     ...
     *
     * @details
     * Callback вызывается из ISR UART TX Complete. Функция должна быть короткой:
     * не выполнять длительных операций, не вызывать передачу в UART и не зависеть
     * от UART-прерываний.
     *
     * @note
     * Используется внешним слоем для действий после полного окончания
     * физической передачи последнего байта.
     */
     ...
     * Линии выбора ведомых устройств должны настраиваться вызывающим кодом.
     */
    void spi_init(void);
