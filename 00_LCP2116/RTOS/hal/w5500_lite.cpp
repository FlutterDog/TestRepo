...
static uint8_t g_w5500_echo_buffer[LCP_ETHERNET_COUNT][W5500_ECHO_BUFFER_SIZE];
...
static void w5500_write_bytes(LcpEthernetId ethernet_id,
                              uint8_t block,
                              uint16_t address,
                              const uint8_t* data,
                              size_t length)
...
static void w5500_read_bytes(LcpEthernetId ethernet_id,
                             uint8_t block,
                             uint16_t address,
                             uint8_t* data,
                             size_t length)
...
static void w5500_write8(LcpEthernetId ethernet_id,
                          uint8_t block,
                          uint16_t address,
                          uint8_t value)
{
...
static uint8_t w5500_read8(LcpEthernetId ethernet_id,
                            uint8_t block,
                            uint16_t address)
{
    uint8_t value = 0U;
...
static void w5500_write16(LcpEthernetId ethernet_id,
                           uint8_t block,
                           uint16_t address,
                           uint16_t value)
{
...
static uint16_t w5500_read16(LcpEthernetId ethernet_id,
                              uint8_t block,
                              uint16_t address)
{
    uint8_t data[2];
...
static void w5500_execute_socket_command(LcpEthernetId ethernet_id,
...
static void w5500_socket_close(LcpEthernetId ethernet_id,
                                uint8_t socket_number)
{
    const uint8_t block = w5500_socket_register_block(socket_number);
...
static void w5500_socket_open_listen(LcpEthernetId ethernet_id,
                                      uint8_t socket_number,
                                      uint16_t port)
{
    const uint8_t block = w5500_socket_register_block(socket_number);
    ...
    {
        w5500_execute_socket_command(ethernet_id,
                                     socket_number,
                                     SN_CR_LISTEN);
    }
}

static uint16_t w5500_socket_recv(LcpEthernetId ethernet_id,
                                   uint8_t socket_number,
                                   uint8_t* buffer,
                                   uint16_t length)
{
    ...
    const uint16_t received_size =
        w5500_read16(ethernet_id, reg_block, SN_RX_RSR);
    ...
    if ((read_size == 0U) || (buffer == 0))
    {
        return 0U;
    }

    const uint16_t rx_read_pointer =
        w5500_read16(ethernet_id, reg_block, SN_RX_RD);
    ...
    w5500_write16(ethernet_id,
                  reg_block,
                  SN_RX_RD,
                  static_cast<uint16_t>(rx_read_pointer + read_size));
    w5500_execute_socket_command(ethernet_id,
                                 socket_number,
                                 SN_CR_RECV);
    return read_size;
...
static uint16_t w5500_socket_send(LcpEthernetId ethernet_id,
                                   uint8_t socket_number,
                                   ...
                                   uint16_t length)
{
    ...
    const uint16_t free_size =
        w5500_read16(ethernet_id, reg_block, SN_TX_FSR);
    ...
    const uint16_t tx_write_pointer =
        w5500_read16(ethernet_id, reg_block, SN_TX_WR);
    ...
    w5500_write16(ethernet_id,
                  reg_block,
                  SN_TX_WR,
                  static_cast<uint16_t>(tx_write_pointer + send_size));
    w5500_execute_socket_command(ethernet_id,
                                 socket_number,
                                 SN_CR_SEND);
    return send_size;
...
static uint8_t w5500_ensure_server_state(LcpEthernetId ethernet_id,
                                          uint16_t port)
{
...
uint16_t w5500_lite_tcp_server_send(LcpEthernetId ethernet_id,
                                    ...
                                    uint16_t length)
{
    if (w5500_lite_tcp_server_status(ethernet_id) != SN_SR_ESTABLISHED)
    {
        return 0U;
    }
...
{
    if (ethernet_id >= LCP_ETHERNET_COUNT)
    {
        return;
    }
    ...
    const uint16_t received = w5500_lite_tcp_server_receive(
        ethernet_id,
        port,
        g_w5500_echo_buffer[ethernet_id],
        W5500_ECHO_BUFFER_SIZE);

    if (received != 0U)
    {
        (void)w5500_lite_tcp_server_send(ethernet_id,
                                         g_w5500_echo_buffer[ethernet_id],
                                         received);
    }
}
