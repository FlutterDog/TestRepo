                           ...
                           uint16_t response_length)
{
    ...
    const uint16_t calculated = modbus_rtu_crc16(master.rx_buffer,
...
void validate_response(ModbusRtuMaster& master)
{
    ...
    if ((master.rx_length >= 2U) &&
        (master.rx_buffer[1] == static_cast<uint8_t>(master.function_code | 0x80U)))
    {
    ...
    if (master.rx_buffer[1] == static_cast<uint8_t>(master.function_code | 0x80U))
    {
                                             ...
                                             uint32_t timeout_ms)
{
    if ((master.state != MODBUS_RTU_MASTER_STATE_IDLE) ||
        (master.result == MODBUS_RTU_RESULT_BUSY) ||
        (transport_valid(master.transport) == 0U) ||
        (slave_address == 0U) ||
        (register_count == 0U) ||
                                               ...
                                               uint32_t timeout_ms)
{
    ...
    if ((master.state != MODBUS_RTU_MASTER_STATE_IDLE) ||
        ...
        ((payload_length + MODBUS_CRC_LENGTH) > MODBUS_RTU_MASTER_TX_CAPACITY) ||
        (timeout_ms == 0U))
    {
        master.result = MODBUS_RTU_RESULT_INVALID_ARGUMENT;
        return 0U;
         ...
         ++register_index)
    {
        ...
        master.tx_buffer[byte_index] = static_cast<uint8_t>(values[register_index] >> 8U);
