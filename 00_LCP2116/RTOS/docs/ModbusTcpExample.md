...
1. Подключить ETH1 или ETH2 к компьютеру или коммутатору.
2. Настроить компьютер в соответствующей подсети.
3. Выполнить `eth` и проверить `VERSIONR=0x04`, `init=ok`, `link=up` и фактический IP.
4. Подключиться Modbus TCP client к порту 502.
5. Выполнить FC03, start address 0, quantity 12.
6. Проверить рост `requests` и `responses` в команде `eth`.
7. Сравнить регистры 0..11 с командой `field`.
8. Повторить тест на втором Ethernet-интерфейсе.
