...
16. [Добавление команды service console](#new-console-command)
...
6. Прошить LCP2116.
7. Подключить USB service console.
...
PC/HMI UART -> diagnostic echo до подключения прикладного протокола
```

> ⚠️ **Внимание**
>
> На одном half-duplex UART нельзя одновременно запускать два protocol engine. Перед подключением нового сервиса необходимо определить владельца порта и отключить прежний режим работы.
...
- service console не должна изменять protocol state machine в обход публичного API;
...
> ⚠️ **Внимание**
>
> Блокирующая функция внутри основной задачи останавливает обслуживание X2X, FieldSensor, Ethernet, RTC, service console и watchdog на всё время ожидания. Не допускаются длительные `delay`, busy-wait и циклы ожидания аппаратного события без timeout.
...
## 8. USB service console

### 8.1 Подключение

Прошивка создаёт USB CDC-порт. Для подключения можно использовать PuTTY, Tera Term, RealTerm или другой serial terminal.
...
> ⚠️ **Внимание**
>
> Непрерывный вывод большого отчёта создаёт дополнительную нагрузку на USB console. Периодический режим следует использовать только при целенаправленной диагностике.
...
## 16. Добавление команды service console
...
> 🟦 **Информация**
>
> Команда service console должна вызывать публичный API соответствующего сервиса, а не изменять его внутреннее runtime-состояние напрямую.
...
### 17.10 Console не принимает команды
...
### 17.11 X2X сначала возвращает exception, затем работает
...
- console сохраняет работоспособность;
- Ethernet и последовательные интерфейсы продолжают обмен одновременно.

> 🟦 **Информация**
>
> Для базового выпуска рекомендуется сохранять итоговый вывод `version`, `status`, `rtos`, `field`, `x2x` и `eth` вместе с результатами сборки Release.
