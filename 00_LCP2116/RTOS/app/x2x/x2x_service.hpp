 ...
 *
 * При ошибке чтения ранее активная конфигурация продолжает работать.
 *
 * @return Результат чтения файла.
 */
X2XConfigResult x2x_service_reload_config(void);

/** @brief Приостанавливает новые транзакции и сбрасывает текущую. */
void x2x_service_pause(void);

/** @brief Возобновляет циклический опрос. */
void x2x_service_resume(void);

/** @brief Возвращает 1, если опрос приостановлен пользователем. */
...
/** @brief Возвращает результат последней попытки чтения X2X.TXT. */
