...
#define adc_quantity		6					// Total ADC quantity
#define osc_length			196					// A number of oscillograms to recordю.Каждый замер 68 мс, чтоб набрать 40 мс нужно сделать 588 замеров и осреднить их на 3 измерения. получим 196 точек
...
class ADC_SPI
{
	...
	void fill_ct_buffer_completely(int16_t osc_buffer[][phase_quantity], uint8_t BLOCK_GAIN_);
	...
	int16_t	overflowOscBuffer[osc_length][phase_quantity];	// An array to keep the oscillograms
	int16_t	calcOscBuffer[osc_length][phase_quantity];
