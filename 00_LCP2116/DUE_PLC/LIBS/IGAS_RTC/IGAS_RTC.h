...
byte const RTC_FLAG_IDLE = 0;
byte const RTC_FLAG_WAIT = 1;
byte const  RTC_FLAG_SET = 2;
byte const  RTC_FLAG_VALID = 3;
byte const  RTC_FLAG_NVAL = 4;
byte const  RTC_FLAG_STBY = 5;
byte const  RTC_FLAG_HMIupd = 6;
...
class RTC_clock
{
	...
	byte updateRTC( Rtc* pRtc, uint8_t wYear, uint8_t ucMonth, uint8_t ucDay,
	...
	uint32_t iDate;  // Converted date format
	uint32_t iTime;
