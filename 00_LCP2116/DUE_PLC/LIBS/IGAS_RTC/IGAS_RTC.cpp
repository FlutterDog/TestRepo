...
byte RTC_clock:: get_years2()
{
	_current_date = current_date();
	return  (((_current_date >> 12) & 0xF) * 10) + ((_current_date >> 8) & 0xF);
}
...
byte const RTC_Debug = 0;


byte RTC_clock :: updateRTC( Rtc* pRtc, uint8_t wYear, uint8_t ucMonth, uint8_t ucDay,
uint8_t ucHour,	uint8_t ucMinute,	uint8_t ucSecond )
{

	// wYear = wYear + 2000;
	
	uint8_t wCent_bcd; 	//set time
	uint8_t ucCent_bcd ; 	//set time
	uint8_t ucYear_bcd ;		//set time
	uint8_t ucMonth_bcd ;	//set time
	uint8_t ucDay_bcd ;	//set time
	uint8_t ucWeek_bcd ;	//set time

	uint8_t ucHour_bcd ; //set time
	uint8_t ucMin_bcd ;	//set time
	uint8_t ucSec_bcd ;	//set time

	uint8_t ucWeek;	//set date
	
	// Calc week number

	ucWeek = calculate_day_of_week(_year, _month, _day);	//set date
	//ucWeek = (ucDay + 2 * ucMonth + 3 * (ucMonth + 1) / 5 + wYear + wYear / 4 - wYear / 100 + wYear / 400) % 7;
	//++ucWeek_bcd;
	
	ucHour_bcd = (ucHour%10)   | ((ucHour/10)<<4) ;	//set time
	ucMin_bcd  = (ucMinute%10) | ((ucMinute/10)<<4) ;	//set time
	ucSec_bcd  = (ucSecond%10) | ((ucSecond/10)<<4) ;	//set time

	
	// value overflow
	if ( (ucHour_bcd & (uint8_t)(~RTC_HOUR_BIT_LEN_MASK)) |	//set time
	(ucMin_bcd & (uint8_t)(~RTC_MIN_BIT_LEN_MASK)) |	//set time
	(ucSec_bcd & (uint8_t)(~RTC_SEC_BIT_LEN_MASK)))	//set time
	{
		return 1 ;
	}
	
	wCent_bcd = 20;
	ucCent_bcd =  (wCent_bcd%10)   | ((wCent_bcd/10)<<4);	//set time
	ucYear_bcd  = (wYear%10)   | ((wYear/10)<<4);	//set time
	ucMonth_bcd = (ucMonth%10)   | ((ucMonth/10)<<4);	//set time
	ucDay_bcd   = (ucDay%10)   | ((ucDay/10)<<4);	//set time
	ucWeek_bcd  = (ucWeek%10)   | ((ucWeek/10)<<4);	//set time

	
	if ( (ucCent_bcd & (uint8_t)(~RTC_CENT_BIT_LEN_MASK)) | //set date
	(ucYear_bcd & (uint8_t)(~RTC_YEAR_BIT_LEN_MASK)) |	//set date
	(ucMonth_bcd & (uint8_t)(~RTC_MONTH_BIT_LEN_MASK)) | //set date
	(ucWeek_bcd & (uint8_t)(~RTC_WEEK_BIT_LEN_MASK)) | //set date
	(ucDay_bcd & (uint8_t)(~RTC_DATE_BIT_LEN_MASK)) //set date
	)
	{
		return 1; //set date
	}
	...
	iDate = ucCent_bcd |(ucYear_bcd << 8) |	(ucMonth_bcd << 16) | (ucWeek_bcd << 21) | (ucDay_bcd << 24); //set date
	iTime = ucSec_bcd | (ucMin_bcd << 8) | (ucHour_bcd<<16) ; 	//set time


	// Added Start
	RTC_DisableIt(RTC, RTC_IDR_ACKDIS);
	
	pRtc->RTC_CR |= (RTC_CR_UPDTIM | RTC_CR_UPDCAL); 	//set time and daate
	//	pRtc->RTC_CR |= RTC_CR_UPDCAL ;		//set date
	
	if (RTC_Debug)
	SerialUSB.println("RTC_FLAG_WAIT -> Set  ");
	
	rtcFlag = RTC_FLAG_WAIT;
...
byte RTC_clock :: rtcLOOP()
{
	// SerialUSB.println("Loop RTC **");
	
	switch (rtcFlag)
	{
		case RTC_FLAG_WAIT:   // Set new Time
		{
			if (RTC_Debug)
			SerialUSB.println("RTC_FLAG_WAIT");
			
			if ((RTC->RTC_SR & RTC_SR_ACKUPD) == RTC_SR_ACKUPD) 	//set time
			{
				rtcFlag = RTC_FLAG_SET;
				
				if (RTC_Debug)
				SerialUSB.println("RTC_FLAG_WAIT -> RTC_FLAG_SET ");
			}
		...
		case RTC_FLAG_SET:
		{
			...
			RTC->RTC_SCCR = RTC_SCCR_ACKCLR ; 	//set time
			RTC->RTC_TIMR = iTime; 	//set time
			RTC->RTC_CALR = iDate ;  //set date

			RTC->RTC_CR &= ((uint32_t)(~RTC_CR_UPDTIM) & (uint32_t)(~RTC_CR_UPDCAL));	//set time
			//	RTC->RTC_CR &= (uint32_t)(~RTC_CR_UPDCAL) ; //set date
			RTC->RTC_SCCR |= RTC_SCCR_SECCLR ;					//set time

			if (RTC_Debug)
			SerialUSB.println("RTC_FLAG_SET case");
			
			if (RTC->RTC_VER & RTC_VER_NVTIM == RTC_VER_NVTIM)  // Check Valid Entry register  	//set time
			{
				if (RTC_Debug)
				SerialUSB.println("RTC_FLAG_SET -> RTC NOT VALID");
				rtcFlag = RTC_FLAG_NVAL;   // Non valid
			}
			
			else
			{
				if (RTC_Debug)
				SerialUSB.println("RTC_FLAG_SET -> RTC_FLAG_VALID");
				rtcFlag = RTC_FLAG_VALID;  // Valid
			}
