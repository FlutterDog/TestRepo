 ...
 */
 ...
 /**
 * @brief	This function established  the Connected for the channel in passive (server) mode. This function waits for the request from the peer.
 * @return	1 for success else 0.
 */
 ...
 /**
 * @brief	This function established  the Connected for the channel in Active (client) mode.
 * 		This function waits for the untill the Connected is established.
 *
 * @return	1 for success else 0.
 */
 ...
 */
 ...
 uint16_t send(SOCKET s, const uint8_t * buf, uint16_t len)
 {
	 ...
	 while ((freesize < ret)  && (micros() - watchD < 5000));

	 if (initSerialDebug)
	 {
		 SerialUSB.println("Sock 135");
	 }
	 ...
	 watchD = micros();
	 while ( ((w5500.readSnIR(s) & SnIR::SEND_OK) != SnIR::SEND_OK) && (micros() - watchD < 5000) )
	 {
 ...
 */
 int16_t recv(SOCKET s, uint8_t *buf, int16_t len)
 {
	 ...
	 if ( ret == 0 )
	 {
		 ...
		 {
			 // The remote end has closed its side of the Connected, so this is the eof state
			 ret = 0;
		 }
		 else
		 {
			 // The Connected is still up, but there's no data waiting to be read
			 ret = -1;
		 }
	 }
	 else if (ret > len)
 ...
 */
 uint16_t sendto(SOCKET s, const uint8_t *buf, uint16_t len, uint8_t *addr, uint16_t port)
 {
	 ...
	 if (len > w5500.SSIZE) ret = w5500.SSIZE; // check size not to exceed MA size.
	 else ret = len;
