...
void IGAS_X2X_485::initMB(int COMM_BPS, int SERIAL_MODE, int allRegs)
{
	Serial1.begin(9600); //(COMM_BPS, SERIAL_MODE);                              // No parity, 1 stop bit
	regLimit = allRegs;
}
...
byte IGAS_X2X_485::update()
{
	...
	if (readyForIncoming) //digitalRead(txEnPin) == HIGH)
	return 0;
...
int IGAS_X2X_485::receive_request(unsigned char *received_string)
{
	...
	/* FIXME: does Serial1.available wait 1.5T or 3.5T before exiting the loop? */
