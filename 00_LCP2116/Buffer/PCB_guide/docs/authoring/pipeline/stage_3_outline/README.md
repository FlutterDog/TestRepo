...
### Шаг один А. Сделать outline визуально читаемым

`outline.md` должен быть не просто набором заголовков, а аккуратно свёрстанным рабочим документом.

Обязательные требования к вёрстке:

* в начале должен быть **нумерованный Contents / Оглавление**;
* нумерация должна быть арабская: `1.`, `1.1.`, `1.1.1.`;
* номера в оглавлении и номера в заголовках тела документа должны совпадать;
* крупные разделы должны быть отделены горизонтальными разделителями `---`;
* после каждого крупного раздела желательно иметь краткий comment / intent block, если это помогает понять логику главы;
* outline не должен выглядеть как рыхлый список строк без визуальной структуры.
...
* draft asset reference path in `IMG/`.

❗ Figures Plan должен быть пригоден не только для planning, но и для прямой вставки clean asset reference blocks в `draft_v0.md` на Stage four.
...
* `outline.md` начинается с нумерованного оглавления и аккуратно свёрстан как рабочий документ;
