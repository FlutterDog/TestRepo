...
class IGAS_X2X_485
{
	...
	uint8_t		update(unsigned long currentTime);
	...
	int			write_regs(uint16_t start_addr, uint8_t * query, int * regs);
	int			preset_multiple_registers(uint8_t slave, uint16_t start_addr, uint8_t count, uint8_t * query, int * regs);
	int			write_single_register(uint8_t slave, uint16_t write_addr, uint8_t * query, int * regs);
	int			read_holding_registers(uint8_t slave, uint16_t start_addr, uint8_t reg_count, int * regs);
