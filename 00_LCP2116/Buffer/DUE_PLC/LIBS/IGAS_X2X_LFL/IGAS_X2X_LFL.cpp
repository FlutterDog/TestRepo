#include "IGAS_X2X_LFL.h"
/* global variables */

SpiUartDevice UARTX;

const byte UART1_CS = 10;
...
//void IGAS_LFL_485::initMB(int COMM_BPS, int SERIAL_MODE, int allRegs)
void IGAS_LFL_485::initMB()
{
	...
	//
	// 		UARTX.begin(9600, 0, UART1_CS);   // PC - RS485 output
	//
	//
	// 		break;
	//
	...
	pinMode(RS1_En, OUTPUT);
	digitalWrite(RS1_En, LOW);
...
unsigned int IGAS_LFL_485::crc(unsigned char * buf, unsigned char start,
unsigned char cnt)
{
	unsigned char i, j;
...
*/
void IGAS_LFL_485::build_read_packet(unsigned char slave, unsigned char function,
...
*/
void IGAS_LFL_485::build_write_packet(unsigned char slave, unsigned char function,
unsigned int start_addr,
unsigned char count,
...
*/
void IGAS_LFL_485::build_write_single_packet(unsigned char slave, unsigned char function,
...
*/
void IGAS_LFL_485::build_error_packet(unsigned char slave, unsigned char function,
...
void IGAS_LFL_485::modbus_reply(unsigned char * packet, unsigned char string_length)
{
	int temp_crc = 0;
...
int IGAS_LFL_485::send_reply(unsigned char * query, unsigned char string_length)
{

	modbus_reply(query, string_length);
	string_length += 2;

	send(query, string_length);
}


int IGAS_LFL_485::send(unsigned char * query, unsigned char string_length)
{
...
int IGAS_LFL_485::receive_request(unsigned char *received_string)
{
...
int IGAS_LFL_485::modbus_request(unsigned char slave, unsigned char * data)
{
...
int IGAS_LFL_485::validate_request(unsigned char * data, unsigned char length)
{
	int i, fcnt = 0;
	...
	if (start_addr > regLimit || (start_addr + regs_num) > regLimit)
	return EXC_ADDR_RANGE;
...
int IGAS_LFL_485::write_regs(unsigned int start_addr, unsigned char * query, int * regs)
{
	int temp = 0;
	unsigned int i = 0;
...
int IGAS_LFL_485::preset_multiple_registers(unsigned char slave,
unsigned int start_addr,
unsigned char count,
unsigned char * query,
int * regs)
...
int IGAS_LFL_485::write_single_register(unsigned char slave,
...
int IGAS_LFL_485::read_holding_registers(unsigned char slave, unsigned int start_addr,
...
void IGAS_LFL_485::setDelegate(void (*fp)(int*,  int,  int, int ) )
{
	fpAction = fp;
}


void IGAS_LFL_485::doAction(int *t1, int t2,int t3, int t4)
{
