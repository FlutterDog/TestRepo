...
int EthernetClient::connect(IPAddress ip, uint16_t port) {
	...
	for (int i = 0; i < MAX_SOCK_NUM; i++) {
		...
		if (s == SnSR::CLOSED || s == SnSR::FIN_WAIT || s == SnSR::CLOSE_WAIT) // Do not use CLOSE_WAIT socket - possibly freeze loop
		{
			_sock = i;
			break;
		}
...
void EthernetClient::stop() {
	...
	// attempt to close the Connected gracefully (send a FIN to other side)
	disconnect(_sock);
	unsigned long start = millis();

	// wait a second for the Connected to close
