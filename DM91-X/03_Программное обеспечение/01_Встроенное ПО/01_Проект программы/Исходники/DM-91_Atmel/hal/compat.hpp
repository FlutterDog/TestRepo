...
namespace hal
{
    ...
    /**
     * @brief Передаёт массив байт ведомому устройству TWI/I2C.
     *
     * @param address Семибитный I2C-адрес устройства.
     * @param data Указатель на передаваемые данные.
     * @param length Количество передаваемых байт.
     *
     * @return true — передача выполнена успешно; false — ошибка обмена.
     *
     * @note
     * При length == 0 функция формирует только адресную фазу записи.
     * Для обычного прикладного кода предпочтительно передавать хотя бы
     * один байт команды или данных.
     */
    bool twi_write(uint8_t address, const uint8_t* data, uint8_t length);

    /**
     * @brief Принимает массив байт от ведомого устройства TWI/I2C.
     *
     * @param address Семибитный I2C-адрес устройства.
     * @param data Указатель на буфер приёма.
     * @param length Количество принимаемых байт.
     *
     * @return true — приём выполнен успешно; false — ошибка обмена.
     *
     * @note
     * При length == 0 обмен по шине не выполняется, функция возвращает true.
     */
    bool twi_read(uint8_t address, uint8_t* data, uint8_t length);

    /**
     * @brief Выполняет комбинированный обмен TWI/I2C: запись, затем чтение.
     *
     * @details
     * Между фазой записи и фазой чтения формируется repeated START.
     * Функция подходит для датчиков, которым сначала передаётся команда,
     * а затем считывается ответ фиксированной длины.
     *
     * @param address Семибитный I2C-адрес устройства.
     * @param tx_data Указатель на передаваемые данные.
     * @param tx_length Количество передаваемых байт.
     * @param rx_data Указатель на буфер приёма.
     * @param rx_length Количество принимаемых байт.
     *
     * @return true — обмен выполнен успешно; false — ошибка обмена.
     *
     * @note
     * Если rx_length == 0, функция работает как обычная запись.
     * Если tx_length == 0, функция работает как обычное чтение.
     */
    bool twi_write_read(
        uint8_t address,
