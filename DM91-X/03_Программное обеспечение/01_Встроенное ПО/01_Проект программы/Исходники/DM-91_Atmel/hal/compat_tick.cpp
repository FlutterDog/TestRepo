 ...
 *
 * @warning
 * Timer0 используется системным тикером. Не следует использовать аппаратный
 * PWM на каналах Timer0 OC0A/OC0B без отдельной переработки этого модуля,
 * так как это может нарушить работу millis() и delay().
 */
