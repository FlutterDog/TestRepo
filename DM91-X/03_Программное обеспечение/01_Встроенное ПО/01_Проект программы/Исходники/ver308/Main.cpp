...
#define FLASH_CONFIG_ADDR 0x7700
...
// вот это заменяет config.h
extern const uint8_t  slaveAddr;
extern const float    densitySetFlt;
extern const uint16_t incomingTimeOut;
extern const uint8_t  baudRate;
extern const uint8_t  parity;
extern const uint16_t serialLow;
extern const uint16_t serialHigh;
extern const uint8_t  hVersion;
extern const uint8_t  sVersion;
extern const uint8_t  modelDN;
extern const uint8_t  getTempFromSHT;
extern const uint8_t  liquefaction;
extern const float    overdensity;
extern const float    warningLevel;
extern const float    alarmLevel;
extern const float    offsetTemp;
extern const float    offsetHumidity_b;
extern const float    offsetHumidity_K;
extern const uint8_t  tian_Pmin;
extern const uint8_t  tian_Pmax;
extern const uint16_t tian_K;
extern const uint16_t tian_Offset;
extern const float    tianTerm1;
extern const float    kP;
extern const float    kI;
extern const float    kD;
extern const int16_t  tempSetPoint;
extern const uint8_t  enableCPUheat;

// … дальше твой код main() …


// uint16_t const hVersion = 7;		// Hardware version
// uint16_t const sVersion = 305; // Software version


// Toggle RS enable pin 10

//uint8_t slaveAddr = 2;		// DM91 slave address
...
#define BOOT_START_ADDR_BYTES  0x7000
#define BOOT_START_ADDR_WORDS  (BOOT_START_ADDR_BYTES / 2)

typedef void (*boot_ptr_t)(void);

// typedef void (*AppPtr_t) (void);
// AppPtr_t GoToBootloader = (AppPtr_t)0x3F07; // standard
// AppPtr_t GoToBootloader = (AppPtr_t)0x3800; // 4 кБ бут
...
void setup()
{
	...
	cli();
	//	slaveAddr = get_modbus_slave_address();
	...
	//	Serial.write(get_modbus_slave_address());
...
void loop_50()
{
	if (currentTime - loop_50_start > loop_50ms)
	{
		...
		if (rebootDM91 && (currentTime - rebootDM91 > 50))
		{
			rebootDM91 = 0;
			jump_to_bootloader();
		}
		...
		switch (registerAddr_)
		{
			...
			*/
			...
			if (writeReg && pinCode)
			{
				...
				//densitySetFlt = 0; //loadFloat(130, 6.176);
			}
			...
			if (writeReg && pinCode)
			{
				...
				//tempSetPoint = 0; //0; //loadFloat(154, -20.0);
			}
			...
			// 			case 770:   // heartbit
			//
			// 			regs[i] = get_modbus_slave_address();
			//
			// 			break;
...
void settings()
{
	...
	// 	if (slaveAddr == 0 || slaveAddr == 0xFF)
	//	 	slaveAddr = 1;

	
	//incomingTimeOut = 8000;
...
//
//
// 	tempSetPoint = 0; //loadFloat(154, -20.0);
//
// 	enableCPUheat = loadByte(159,0);
//
//
// 	incomingTimeOut = loadInt(161, 8000);
//
// }
...
// 	return eepromBuffer;
//
// 	if (eepromBuffer == 0xFF || eepromBuffer == 0)
// 	{
...
// float 0; //loadFloat(int addr_, float default_)
// {
// 	floatval castFloat_;
// 	uint8_t error_ = 0;
//
...
uint8_t get_modbus_slave_address(void) {
	uint8_t addr = pgm_read_byte_near(FLASH_CONFIG_ADDR);

	// если значение пустое (Flash по умолчанию FF), выбираем 1
...
static inline void jump_to_bootloader(void)
{
	...
	MCUCR = _BV(IVCE);
	MCUCR = _BV(IVSEL);
	((boot_ptr_t)BOOT_START_ADDR_WORDS)(); // слововый адрес
}
