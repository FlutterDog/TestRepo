...
unsigned long incomingWDT = 0;
uint16_t const loop_500ms = 500;
uint16_t const incomingTimeOut = 5000;
uint8_t pinCode = 0;
...
void loop_5000()
{
	if (currentTime - incomingWDT > incomingTimeOut) // если входящих сообщений нет более incomingTimeOut - значит возможно сенсор завис. перезагружаем CPU
	{
...
void modbusProceed(int *regs,  int start_addr_,  int reg_count_, int Value_)
{
	...
	incomingWDT = currentTime;  // если есть адрессное входящее сообщение - сбрасываем таймер WDT
