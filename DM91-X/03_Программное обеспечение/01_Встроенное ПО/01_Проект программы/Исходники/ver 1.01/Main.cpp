...
uint8_t shtSensorFailure = 0;	// Humidity sensor fault flag
...
void checkError()
{
	...
	if (pressGaugeLost == 1)		// Comm error
	{
		pressGaugeLost = 2;
		errorInt |= 1 << 5;
		pressureTian = -100.0;
...
void getTian()  // прочитать данные с сенсора WT - давление и температура
{
	...
	// 	I2c.read(addrWT,0x50,6); //read 6 bytes (x,y,z) from the device
	//
	// 	for (uint8_t i = 0; i < 4; i++) {
	// 		BufWT[i] = I2c.receive();
	// 	}
	...
	pressureTian = (BufWT[0] & B00111111)<<8 | BufWT[1];
	
	//	pressureTian = 1 + 10*((pressureTian-1638)*(2+0.1)/13110-0.1);
	// pressureTian = 1.0 + ((pressureTian-tian_Offset)*(tian_Pmax+tian_Pmin)/tian_K-tian_Pmin);
	
	
	pressureTian = 100.0 + 100.0*(((pressureTian-tian_Offset) * tianTerm1) - tian_Pmin);
	
	temperatureTian = (BufWT[2]<<3) | ((BufWT[3] >> 5) & B00000111);
	
	temperatureTian = offsetTemp + 200*temperatureTian/2047-50;
...
void getDewPoint()
{
	if (shtSensorFailure == 0)  // Sensor ok
	{
