"""Capture non-destructive LCP diagnostic console reports."""
...
from lorentz_test.protocols.lcp_console import LcpDiagnosticConsole, parse_key_value_output
...
def _capture_command(
    ...
    try:
        ...
        return DiagnosticCommandResult(
            ...
            parsed_values=parse_key_value_output(raw_output),
...
def run_lcp_diagnostic_snapshot(
    ...
    *,
    ...
    """Capture raw reports without applying unverified hardware thresholds."""
    ...
    try:
        ...
        for command_id, title, command in _DIAGNOSTIC_COMMANDS:
            commands.append(_capture_command(console, command_id, title, command))

        result = (
            TestStatus.PASS
            if all(item.status == TestStatus.PASS for item in commands)
